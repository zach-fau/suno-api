...
import {BrowserContext, chromium, firefox, Locator, Page} from 'rebrowser-playwright-core';
...
class SunoApi {
    ...
    private async launchBrowser(): Promise<BrowserContext> {
        ...
        logger.debug('Setting browser cookies:', cookies.map(c => ({
            name: c.name,
            domain: c.domain,
            sameSite: c.sameSite,
            httpOnly: c.httpOnly
     ...
     * @param persona_id - Optional persona ID to use for voice/style consistency
     * @returns Promise resolving to array of AudioInfo objects (typically 2 variations)
